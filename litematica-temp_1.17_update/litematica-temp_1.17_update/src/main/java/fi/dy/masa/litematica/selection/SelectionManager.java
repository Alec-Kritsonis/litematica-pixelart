...
import fi.dy.masa.litematica.gui.GuiAreaSelectionEditorPixelArt;
...
public class SelectionManager
{
    ...
    public boolean renameImagePath(String selectionId, String path, IMessageConsumer feedback)
    {
        ...
        if (selection != null)
        {
            selection.setImagePath(path);

            this.selections.put(selectionId, selection);

            return true;
        }
        feedback.addMessage(MessageType.ERROR, "Could not set file path for some god forsaken reason. Path: " + path);
        return false;
    }
    ...
    @Nullable
    public GuiBase getPixelGui()
    {
        AreaSelection selection = this.getCurrentSelection();

        return new GuiAreaSelectionEditorPixelArt(selection);
